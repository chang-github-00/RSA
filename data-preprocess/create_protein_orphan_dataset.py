...
def download_and_preprocess(pdb_id, chain_name, store_path):
    ...
    if chain is not None:
        ...
        if len(seq) == len(tertiary):
            ...
            length = min(len(seq), len(tertiary))
            return seq[:length], tertiary[:length]
    else:
        return None, None
...
if __name__ == '__main__':
    path = '../tape/'
    de_novo_name_file = path + 'denovo/orphan_target_chains.txt'
    ...
    storeLMDB(cache, '../tape/data/proteinnet/proteinnet_orphan.lmdb')
    data = read_lmdb('../tape/data/proteinnet/proteinnet_orphan.lmdb')
