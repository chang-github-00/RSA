...
from msa import MSAForSequenceClassification,MSAForTokenClassification,MSAForProteinContactPrediction,MSAForInteractionPrediction
...
class BertForProteinContactPrediction(BertPreTrainedModel):
    def __init__(self, config, mean_output):
        ...
        #self.init_weights()  init话看上去没用pretrained
    
    def forward(self, input_ids, protein_length, attention_mask=None, labels=None, **kwargs):
        targets = labels
...
class ESMForProteinContactPrediction(ESMPreTrainedModel):
    def __init__(self, config, mean_output):
        super().__init__(config)
        self.num_labels = config.num_labels
...
class BertForTokenClassification2(BertPreTrainedModel):
    ...
    def forward(
        ...
        #print("length is:", protein_length)
        outputs = self.bert(
            input_ids,
            attention_mask=None,
            token_type_ids=token_type_ids,
...
class BertForInteractionPrediction(BertPreTrainedModel):
    def __init__(self, config, mean_output):
        super().__init__(config)
        self.num_labels = config.num_labels
    ...
    def forward(self, input_ids_1, input_ids_2, protein_length_1=None, protein_length_2=None, attention_mask_1=None, attention_mask_2=None, labels=None, output_attentions=None, output_hidden_states=None, return_dict=True,**kwargs):
        return_dict = return_dict if return_dict is not None else self.config.use_return_dict
        ...
        if self.mean_output is not True:
            ...
            num_batch_size = attention_mask_1.size(0)
            outputs_1_ = torch.stack([outputs_1_.last_hidden_state[i, attention_mask_1[i, :], :].mean(dim=0) for i in
                                      range(num_batch_size)], dim=0)
        ...
        if self.mean_output is not True:
            ...
            num_batch_size = attention_mask_2.size(0)
            outputs_2_ = torch.stack([outputs_2_.last_hidden_state[i, attention_mask_2[i, :], :].mean(dim=0) for i in
                                      range(num_batch_size)], dim=0)
...
model_mapping = {
    'bert':{
        ...
        'contact': BertForProteinContactPrediction,
        ...
        'subcellular': BertForSequenceClassification2,
        'ppi': BertForInteractionPrediction
    },
    'esm':{
        'remote_homology': ESMForSequenceClassification,
        'contact': ESMForProteinContactPrediction,
        'fluorescence': ESMForSequenceClassification,
        'stability': ESMForSequenceClassification,
        'ss3': ESMForTokenClassification,
        'ss8': ESMForTokenClassification,
        'subcellular': BertForSequenceClassification2
    },
    'msa': {
        'remote_homology': MSAForSequenceClassification,
        'contact': MSAForProteinContactPrediction,
        'fluorescence': MSAForSequenceClassification,
        'stability': MSAForSequenceClassification,
        'ss3': MSAForTokenClassification,
        'ss8': MSAForTokenClassification,
        'subcellular': MSAForSequenceClassification,
        'ppi':MSAForInteractionPrediction
    }
