...
def compute_accuracy_metrics(task_name, preds, label_ids):
    return {
        "accuracy": accuracy_score_remote(label_ids, preds)
    }


def compute_spearmanr_metrics(task_name, preds, label_ids):
    print(label_ids.shape, preds.shape)
    return{
        "spearmanr": spearmanr(label_ids, preds)
    }
...
def build_compute_metrics_fn(task_name: str, output_type: str) -> Callable[[EvalPrediction], Dict]:
    def compute_metrics_fn(p: EvalPrediction):
        ...
        elif output_type == 'sequence-level-regression':
            logits = p.predictions
            preds = logits
            label_ids = p.label_ids
            return compute_metrics_mapping[task_name](task_name, preds, label_ids)
        elif output_type == 'sequence-level-classification':
            logits = p.predictions
            preds = logits
            label_ids = p.label_ids
            return compute_metrics_mapping[task_name](task_name, preds, label_ids)
...
compute_metrics_mapping = {
    ...
    'contact': None,
    'subcellular': compute_accuracy_metrics,
    'ppi': compute_accuracy_metrics
